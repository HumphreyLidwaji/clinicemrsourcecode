...
// AUDIT LOG: Access attempt for activating imaging
audit_log($mysqli, [
    ...
    'patient_id'  => null,
    'visit_id'    => null,
    'description' => "Attempting to access imaging activation for imaging ID: " . $imaging_id,
    'status'      => 'ATTEMPT',
    'old_values'  => null,
    'new_values'  => null
]);
...
// Get imaging details before activation for audit log
...
// AUDIT LOG: Attempt to activate imaging
audit_log($mysqli, [
    'user_id'     => $_SESSION['user_id'] ?? null,
    'user_role'   => $_SESSION['user_role_name'] ?? 'UNKNOWN',
    'action'      => 'ACTIVATE',
    ...
    'patient_id'  => null,
    'visit_id'    => null,
    'description' => "Attempting to activate radiology imaging: " . $imaging['imaging_name'] . " (" . $imaging['imaging_code'] . ")",
    'status'      => 'ATTEMPT',
...
try {
    // Activate the imaging
    $update_sql = "UPDATE radiology_imagings SET is_active = 1, updated_by = ?, updated_at = NOW() WHERE imaging_id = ?";
    $update_stmt = $mysqli->prepare($update_sql);
    $update_stmt->bind_param("ii", $session_user_id, $imaging_id);
    
    if (!$update_stmt->execute()) {
        throw new Exception("Error activating radiology imaging: " . $mysqli->error);
    }
    $update_stmt->close();

    // If there's an associated billable item, activate it too
    if ($has_billable_item && $billable_item) {
        $update_billable_sql = "UPDATE billable_items SET is_active = 1, updated_by = ?, updated_at = NOW() WHERE item_id = ?";
        ...
        if (!$update_billable_stmt->execute()) {
            throw new Exception("Error activating associated billable item: " . $mysqli->error);
        }
        $update_billable_stmt->close();
        
        // AUDIT LOG: Billable item activation
        audit_log($mysqli, [
            'user_id'     => $_SESSION['user_id'] ?? null,
            'user_role'   => $_SESSION['user_role_name'] ?? 'UNKNOWN',
            'action'      => 'ACTIVATE',
            ...
            'patient_id'  => null,
            'visit_id'    => null,
            'description' => "Activated billable item for radiology imaging: " . $imaging['imaging_name'] . " (" . $imaging['imaging_code'] . ")",
            'status'      => 'SUCCESS',
            'old_values'  => json_encode(['is_active' => $billable_item['is_active']]),
            'new_values'  => json_encode(['is_active' => 1, 'updated_by' => $session_user_id, 'updated_at' => date('Y-m-d H:i:s')])
        ]);
    }
    ...
    $activity_desc = "Activated radiology imaging: " . $imaging['imaging_name'] . " (" . $imaging['imaging_code'] . ")";
    if ($has_billable_item) {
        $activity_desc .= ". Associated billable item (ID: " . $billable_item['item_id'] . ") also activated.";
    }
    ...
    // AUDIT LOG: Successful imaging activation
    audit_log($mysqli, [
        'user_id'     => $_SESSION['user_id'] ?? null,
        'user_role'   => $_SESSION['user_role_name'] ?? 'UNKNOWN',
        'action'      => 'ACTIVATE',
        ...
        'patient_id'  => null,
        'visit_id'    => null,
        'description' => "Successfully activated radiology imaging: " . $imaging['imaging_name'] . " (" . $imaging['imaging_code'] . ")" . 
    ...
    $_SESSION['alert_message'] = "Radiology imaging activated successfully!" . 
                                ($has_billable_item ? " Associated billable item also activated." : "");
    ...
    audit_log($mysqli, [
        ...
        'patient_id'  => null,
        'visit_id'    => null,
        'description' => "Failed to activate radiology imaging: " . $imaging['imaging_name'] . ". Error: " . $e->getMessage(),
        'status'      => 'FAILED',
        'old_values'  => json_encode($old_imaging_data),
        'new_values'  => json_encode($new_imaging_data)
    ]);
