...
$selected_ward_id = $_POST['ward_id'] ?? $_SESSION['selected_ward_id'] ?? null;

if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['ward_id'])) {
    $_SESSION['selected_ward_id'] = intval($_POST['ward_id']);
    $selected_ward_id = $_SESSION['selected_ward_id'];
}
    ...
    <div class="card-header bg-primary py-2">
        <div class="d-flex justify-content-between align-items-center">
            <h3 class="card-title mt-2 mb-0">
                <i class="fas fa-fw fa-user-nurse mr-2"></i>
                Ward Overview - Nurse Station
                <small class="ml-2 badge badge-light">View Only</small>
            </h3>
        </div>
    </div>
            ...
            <div class="card mb-4">
                <div class="card-header bg-light">
                    <h5 class="card-title mb-0">
                        ...
                        <span class="badge badge-primary ml-2"><?php echo $beds_count; ?> beds</span>
                    </h5>
                </div>
                <div class="card-body">
                    <!-- Ward Info -->
                    ...
                    <div class="row">
                        <?php while($bed = $beds_stmt->fetch_assoc()): 
                            ...
                            if (!empty($bed['patient_first_name']) && !empty($bed['patient_last_name'])) {
                                $patient_name = htmlspecialchars($bed['first_name'] . ' ' . $bed['last_name']);
                                $patient_info = calculateAge($bed['date_of_birth']) . ' • ' . $bed['sex'];
                            }
                        ?>
                                        ...
                                        <?php if (!empty($bed['bed_last_cleaned_at'])): ?>
                                            <p class="mb-0">
                                                <small class="text-success">
                                                    ...
                                                    Cleaned: <?php echo date('M j, g:i A', strtotime($bed['bed_last_cleaned_at'])); ?>
                                                </small>
                                            </p>
                                        <?php else: ?>
                                            <p class="mb-0">
