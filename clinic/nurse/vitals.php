...
if ($visit_id <= 0) {
    ...
    audit_log($mysqli, [
        ...
        'patient_id'  => null,
        'visit_id'    => null,
        'description' => "Attempted to access vitals.php with invalid visit ID: " . $visit_id,
        'status'      => 'FAILED',
        'old_values'  => null,
        'new_values'  => null
    ]);
...
if ($result->num_rows === 0) {
    ...
    audit_log($mysqli, [
        ...
        'patient_id'  => null,
        'visit_id'    => $visit_id,
        'description' => "Attempted to access vitals for visit ID " . $visit_id . " but visit not found",
        'status'      => 'FAILED',
        'old_values'  => null,
        'new_values'  => null
    ]);
...
// AUDIT LOG: Successful access to vitals page
audit_log($mysqli, [
    'user_id'     => $_SESSION['user_id'] ?? null,
    'user_role'   => $_SESSION['user_role_name'] ?? 'UNKNOWN',
    'action'      => 'VIEW',
    ...
    'visit_id'    => $visit_id,
    'description' => "Accessed vitals page for visit ID " . $visit_id . " (Patient: " . $patient_info['first_name'] . " " . $patient_info['last_name'] . "). Visit type: " . $visit_type,
    'status'      => 'SUCCESS',
    'old_values'  => null,
    'new_values'  => null
]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_vitals'])) {
    ...
    // Prepare vitals data for audit log
    $vitals_data = [
    ...
    audit_log($mysqli, [
        ...
        'patient_id'  => $patient_info['patient_id'],
        'visit_id'    => $visit_id,
        'description' => "Attempting to record new vitals for patient: " . $patient_info['first_name'] . " " . $patient_info['last_name'],
        'status'      => 'ATTEMPT',
        'old_values'  => null,
        'new_values'  => $vitals_data
    ]);
    ...
    $insert_sql = "INSERT INTO vitals 
                  (patient_id, visit_id, visit_type, recorded_at, 
                   ...
                   bmi, remarks, recorded_by) 
                  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    
    $recorded_by = $_SESSION['user_id'] ?? null;
    
    $insert_stmt = $mysqli->prepare($insert_sql);
    $insert_stmt->bind_param("iisssiiiiidddsi",
    ...
    if ($insert_stmt->execute()) {
        $vitals_id = $insert_stmt->insert_id;
        
        // AUDIT LOG: Successful vitals creation
        audit_log($mysqli, [
            'user_id'     => $_SESSION['user_id'] ?? null,
            'user_role'   => $_SESSION['user_role_name'] ?? 'UNKNOWN',
            'action'      => 'VITALS_CREATE',
            ...
            'patient_id'  => $patient_info['patient_id'],
            'visit_id'    => $visit_id,
            'description' => "Vitals recorded successfully for patient: " . $patient_info['first_name'] . " " . $patient_info['last_name'],
            'status'      => 'SUCCESS',
            'old_values'  => null,
            'new_values'  => array_merge($vitals_data, [
                'vitals_id' => $vitals_id,
                'patient_id' => $patient_info['patient_id'],
                'visit_id' => $visit_id,
        ...
        // AUDIT LOG: Failed vitals creation
        audit_log($mysqli, [
            'user_id'     => $_SESSION['user_id'] ?? null,
            'user_role'   => $_SESSION['user_role_name'] ?? 'UNKNOWN',
            'action'      => 'VITALS_CREATE',
            ...
            'patient_id'  => $patient_info['patient_id'],
            'visit_id'    => $visit_id,
            'description' => "Failed to record vitals. Error: " . $mysqli->error,
            'status'      => 'FAILED',
            'old_values'  => null,
            'new_values'  => null
        ]);
        ...
        $_SESSION['alert_type'] = "error";
        $_SESSION['alert_message'] = "Error recording vitals: " . $mysqli->error;
    }
}

// Handle deletion of vitals (if needed)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_vitals'])) {
    $vitals_id = intval($_POST['vitals_id']);
    
    // Get current vitals details for audit log
    $current_vitals_sql = "SELECT * FROM vitals WHERE vitals_id = ?";
    $current_vitals_stmt = $mysqli->prepare($current_vitals_sql);
    $current_vitals_stmt->bind_param("i", $vitals_id);
    $current_vitals_stmt->execute();
    $current_vitals_result = $current_vitals_stmt->get_result();
    $current_vitals = $current_vitals_result->fetch_assoc();
    
    if ($current_vitals) {
        // AUDIT LOG: Delete vitals attempt
        audit_log($mysqli, [
            'user_id'     => $_SESSION['user_id'] ?? null,
            'user_role'   => $_SESSION['user_role_name'] ?? 'UNKNOWN',
            ...
            'patient_id'  => $patient_info['patient_id'],
            'visit_id'    => $visit_id,
            'description' => "Attempting to delete vitals record ID: " . $vitals_id . " recorded at " . $current_vitals['recorded_at'],
            'status'      => 'ATTEMPT',
            'old_values'  => $current_vitals,
            'new_values'  => null
        ]);
        
        $delete_sql = "DELETE FROM vitals WHERE vitals_id = ?";
        $delete_stmt = $mysqli->prepare($delete_sql);
        $delete_stmt->bind_param("i", $vitals_id);
        
        if ($delete_stmt->execute()) {
            // AUDIT LOG: Successful vitals deletion
            audit_log($mysqli, [
                'user_id'     => $_SESSION['user_id'] ?? null,
                'user_role'   => $_SESSION['user_role_name'] ?? 'UNKNOWN',
                'action'      => 'VITALS_DELETE',
                ...
                'patient_id'  => $patient_info['patient_id'],
                'visit_id'    => $visit_id,
                'description' => "Vitals deleted successfully. Record ID: " . $vitals_id . " recorded at " . $current_vitals['recorded_at'],
                'status'      => 'SUCCESS',
                'old_values'  => $current_vitals,
                'new_values'  => null
            ]);
            ...
            $_SESSION['alert_type'] = "success";
            $_SESSION['alert_message'] = "Vitals record deleted successfully";
            header("Location: vitals.php?visit_id=" . $visit_id);
            exit;
        } else {
            // AUDIT LOG: Failed vitals deletion
            audit_log($mysqli, [
                'user_id'     => $_SESSION['user_id'] ?? null,
                'user_role'   => $_SESSION['user_role_name'] ?? 'UNKNOWN',
                'action'      => 'VITALS_DELETE',
                ...
                'patient_id'  => $patient_info['patient_id'],
                'visit_id'    => $visit_id,
                'description' => "Failed to delete vitals record. Error: " . $mysqli->error,
                'status'      => 'FAILED',
                'old_values'  => null,
                'new_values'  => null
            ]);
            ...
            $_SESSION['alert_type'] = "error";
            $_SESSION['alert_message'] = "Error deleting vitals record: " . $mysqli->error;
        }
    }
}
